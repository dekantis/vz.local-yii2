...
<?php
if(!$isNewRecord) {
    $append = "updateAvailableHour('{$model->hour}')";
} else {
    $append = '';
}
$js = <<<JS
var updateAvailableHour = function(selectedValue = null) {
    var url = '/user/record/available-hours';
    var hours = $('#hours');
    var date = $('#date').val();
    ...
    $.get(url + '?date=' + date + '&clinic_id=' + clinicId, function(data) {
        ...
        if(data == []) {
            ...
            $.each(data, function(index, value) {
                ...
                hours.append('<option' + selected + ' value="' + index + '">' + value + '</options>');
            });
        }
    }, 'json');
}
...
$this->registerJS($append, $this::POS_READY)
